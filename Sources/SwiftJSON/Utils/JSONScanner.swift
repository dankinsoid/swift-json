...
struct JSONScanner {
	...
	var currentIndex: UnsafeBufferPointer<UInt8>.Index { index }
	...
	mutating func peekOneCharacter() throws -> Character {
		skipWhitespace()
	...
	private func parseBareDouble(
		...
		try parseBareNumber(source: source, index: &index, end: end, parse: numberFormatter.utf8ToDouble)
	}
	...
	mutating func nextBytesValue() throws -> Data {
		skipWhitespace()
		guard hasMoreContent else {
			throw JSONDecodingError.truncated
		}
	...
	mutating func nextUnsignedInteger<U: FixedWidthInteger & UnsignedInteger>() throws -> U {
		skipWhitespace()
		guard hasMoreContent else {
			throw JSONDecodingError.truncated
		}
	...
	/// unless the number is written in floating-point format.  In that
	/// case, we decode it with only Double precision.
	mutating func nextSignedInteger<I: SignedBitPatternInitializable>() throws -> I {
		skipWhitespace()
		guard hasMoreContent else {
			throw JSONDecodingError.truncated
		}
	...
	/// quoted strings.
	mutating func nextFloat() throws -> Float {
		skipWhitespace()
		guard hasMoreContent else {
			throw JSONDecodingError.truncated
		}
	...
	mutating func nextQuotedString() throws -> String {
		skipWhitespace()
		guard hasMoreContent else {
			throw JSONDecodingError.truncated
		}
	...
	mutating func nextOptionalQuotedString() throws -> String? {
		skipWhitespace()
		guard hasMoreContent else {
			return nil
		}
	...
	mutating func skipOptionalNull() -> Bool {
		skipWhitespace()
	...
	/// keys, for instance.)
	mutating func nextBool() throws -> Bool {
		skipWhitespace()
		guard hasMoreContent else {
			throw JSONDecodingError.truncated
		}
	...
	/// full processing of quoted boolean values.  (Used in map
	/// keys, for instance.)
	mutating func nextQuotedBool() throws -> Bool {
		skipWhitespace()
		guard hasMoreContent else {
			throw JSONDecodingError.truncated
		}
...
//
	...
	mutating func skipRequiredObjectStart() throws {
		try skipRequiredCharacter(asciiOpenCurlyBracket) // {
		try incrementRecursionDepth()
	}

	/// Skip ",", throw if that's not the next character
	mutating func skipRequiredComma() throws {
		try skipRequiredCharacter(asciiComma)
	}

	/// Skip ":", throw if that's not the next character
	mutating func skipRequiredColon() throws {
		try skipRequiredCharacter(asciiColon)
	}

	/// Skip "[", throw if that's not the next character
	mutating func skipRequiredArrayStart() throws {
	...
	mutating func skipOptionalArrayEnd() -> Bool {
		skipOptionalCharacter(asciiCloseSquareBracket) // ]
	}

	/// If the next non-whitespace character is "}", skip it
	/// and return true.  Otherwise, return false.
	mutating func skipOptionalObjectEnd() -> Bool {
	...
	mutating func skip() throws -> UnsafeBufferPointer<UInt8> {
		skipWhitespace()
		let start = index
		try skipValue()
	...
	mutating func set(index: UnsafeBufferPointer<UInt8>.Index) {
		self.index = index
	}
}
